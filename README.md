...
the training run. It also checks its own findings: every run is repeated at the
same seed to measure the noise floor, the closest call is settled with three
seeds, and the context-length axis is scored a second, fairer way, which moves
the answer from 128 characters to 64. Supporting material lives in `results/`:

| File | What it is |
|---|---|
| `results/figures/` | loss curve and ablation charts |
| `results/experiments/<run>/` | eval and step logs for every ablation run, the reported numbers |
| `results/experiments_rerun/<run>/` | the same eleven runs repeated at the same seed on an idle machine: clean timings and the noise floor |
| `results/experiments_seeds/<run>/` | chunk sizes 64, 128 and 256 at two more seeds |
| `results/common_context_eval.csv` | every chunk size scored on identical targets with full context |
| `results/common_context_seeds.csv` | the same fair scoring across all three seeds |
...
| `results/finetune/` | phase 2: SmolLM2 fine-tune logs, base and fine-tuned samples, per-source loss for both |
| `results/finetune_qwen/` | phase 2: Qwen3-1.7B LoRA logs and per-source loss, base and fine-tuned |
| `results/playground.json` | the saved outputs of all five models on the page's eight prompts |

## Phase 2: fine-tuning pretrained models
...
`docs/` with GitHub Pages. It is generated by `site/build.py` from
